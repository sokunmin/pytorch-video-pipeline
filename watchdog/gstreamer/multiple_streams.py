...
class VideoStream():
    ...
    def __init__(self, src_id, config):
        """Summary
    ...
    def build_pipeline(self, config):
        ...
        if config.appsink.mode == 'gst':
            pipeline = self.build_pipeline_branch(self.src_id, config.uri[self.src_id], config.appsink.seg_maxtime)
        ...
        if config.appsink.mode == 'gst':
            self.connect_sink(src_id=self.src_id)
...
def concurrent_fill_frames(sources):
    # Create four thread pools and assign values​to the corresponding positions of the placeholder images
    num_src = len(sources)
    with futures.ThreadPoolExecutor(num_src) as executor:
        res = executor.map(fill_frame, [s.frame() for s in sources])
        tiled_image = np.zeros([CELL_H * 2, CELL_W * 2, 3], np.uint8)
        for i, f in enumerate(res):
            cell = np.zeros([CELL_H, CELL_W, 3], np.uint8)
            h, w, _ = f.shape
            cell[:h, :w, :] = f
            if i == 0:
                tiled_image[:CELL_H, :CELL_W] = cell
            if i == 1:
                tiled_image[:CELL_H, CELL_W:] = cell
            if i == 2:
                tiled_image[CELL_H:, :CELL_W] = cell
...
if __name__ == '__main__':
    ...
    if args.uri is not None:
        ...
        if not isinstance(cfg.uri, (tuple, list)):
            cfg.uri = [cfg.uri]
    sources = [VideoStream(src_id=i, config=cfg) for i in range(len(cfg.uri))]

    while True:
        # Wait for the next frame
        all_valid = [s.frame_available() for s in sources]
        if not all(all_valid):
            continue

        tiled_image = concurrent_fill_frames(sources)
